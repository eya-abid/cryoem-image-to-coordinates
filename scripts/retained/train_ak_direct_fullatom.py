...
def load_coords_from_pdb(pdb_path: str, max_atoms: int | None = None, atom_mode: str = "cap") -> np.ndarray:
    ca_coords = []
    p_coords = []
    all_atoms = []
        ...
        for line in f:
            ...
            if atom_mode == "all":
                ...
                if atom_name == "CA":
                    ...
                    p_coords.append(xyz)
    arr = np.asarray(all_atoms if atom_mode == "all" else (ca_coords + p_coords), dtype=np.float32)
...
class SpiImageDataset(Dataset):
    def __init__(self, pairs: List[Tuple[str, str, str]], max_atoms: int, image_hw: Tuple[int, int], atom_mode: str):
        self.pairs = pairs
    ...
    def __getitem__(self, idx: int):
        ...
        coords = load_coords_from_pdb(pdb_path, self.max_atoms, atom_mode=self.atom_mode)
...
class StrongCNNToCoords(nn.Module):
    def __init__(self, max_atoms: int, hidden_dim: int = 2048, mlp_layers: int = 4, dropout: float = 0.15):
        super().__init__()
...
def write_pdb_from_template(template_path: Path, coords: np.ndarray, output_path: Path):
    coord_idx = 0
    out_lines = []
    with template_path.open("r") as handle:
        for line in handle:
            if line.startswith("ATOM") and coord_idx < len(coords):
...
def train(args):
    ...
    ds_train = SpiImageDataset(train_pairs, args.max_atoms, (args.image_h, args.image_w), args.atom_mode)
    ds_val = SpiImageDataset(val_pairs, args.max_atoms, (args.image_h, args.image_w), args.atom_mode)
    ...
    model = StrongCNNToCoords(args.max_atoms, hidden_dim=args.hidden_dim, mlp_layers=args.mlp_layers, dropout=args.dropout).to(device)
...
def infer(args):
    ...
    ds = SpiImageDataset(infer_pairs, args.max_atoms, (args.image_h, args.image_w), args.atom_mode)
    ...
    with torch.no_grad():
        for batch in dl:
            ...
            for b in range(pred.size(0)):
                ...
                write_pdb_from_template(Path(batch["pdb_path"][b]), coords_np, pred_dir / f"{sid}_pred.pdb")
...
def main():
    ...
    ap.add_argument("--atom_mode", choices=["cap", "all"], default="cap")
